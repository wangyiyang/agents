...
- 🎯 Entry points to execute your program using `python -m <agents>` or `$ agents` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/wangyiyang/agents/branch/main/graph/badge.svg?token=agents_token_here)](https://codecov.io/gh/wangyiyang/agents)
[![CI](https://github.com/wangyiyang/agents/actions/workflows/main.yml/badge.svg)](https://github.com/wangyiyang/agents/actions/workflows/main.yml)

Awesome agents created by wangyiyang

## Install

from source
```bash
git clone https://github.com/wangyiyang/agents agents
cd agents
make install
```
...
```bash
$ agents run --port 8080
```

or

```bash
python -m agents run --port 8080
```
...
```bash
❯ agents create-user --help
Usage: agents create-user [OPTIONS] USERNAME PASSWORD
...
```bash
agents create-user admin admin --superuser
```
...
Run with `agents run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `agents create-user`.
...
agents/app.py                  16      1    94%
agents/cli.py                  21      0   100%
agents/config.py                5      0   100%
agents/db.py                   10      0   100%
agents/models/__init__.py       0      0   100%
agents/models/content.py       47      1    98%
agents/routes/__init__.py      11      0   100%
agents/routes/content.py       52     25    52%
agents/routes/security.py      15      1    93%
agents/routes/user.py          52     26    50%
agents/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in agents/default.toml.
